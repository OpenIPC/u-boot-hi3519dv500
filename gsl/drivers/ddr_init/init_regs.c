...
static void reg_read(const struct regentry *reg, unsigned int *ret)
{
	...
	reg_val_r = (*(volatile unsigned *)(unsigned long)(reg->reg_addr));
	if (bit_num_r != BIT_CNT_32) {
...
static void reg_write(const struct regentry *reg)
{
	...
	reg_val_w = (*(volatile unsigned *)(unsigned long)(reg->reg_addr));
	if (bit_num_w == BIT_CNT_32) {
		reg_val_w = reg->value;
	} else {
