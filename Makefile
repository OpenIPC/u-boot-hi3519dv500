...
export ARCH CPU BOARD VENDOR SOC CPUDIR BOARDDIR VENDER_CHIP
...
ifdef CONFIG_SECURITY_SUBSYS_ENABLE
libs-$(CONFIG_SECURITY_SUBSYS_ENABLE) += product/security_subsys/
libs-$(CONFIG_WRITE_OTP_CMD) += product/write_otp/
endif
...
GAPFILL := --gap-fill=0xff
endif
# Normally we fill empty space with 0xff
quiet_cmd_objcopy = OBJCOPY $@
cmd_objcopy = $(OBJCOPY) $(GAPFILL) $(OBJCOPYFLAGS) \
	$(OBJCOPYFLAGS_$(@F)) $< $@
...
OBJCOPYFLAGS_u-boot-rockchip.bin = -I binary -O binary \
	--pad-to=$(CONFIG_SPL_PAD_TO) $(GAPFILL)
...
OBJCOPYFLAGS_u-boot-img-spl-at-end.bin := -I binary -O binary \
	--pad-to=$(CONFIG_UBOOT_PAD_TO) $(GAPFILL)
...
clean: $(clean-dirs) ddr_training_clean
	...
	@find $(if $(KBUILD_EXTMOD), $(KBUILD_EXTMOD), .) $(RCS_FIND_IGNORE) \
		...
		-o -name '*.ko.*' -o -name '*.su' -o -name '*.d' -o -name '*.pyc' \
...
ddr_training_prepare:
	make -C $(srctree)/drivers/ddr/vendor/boot_adaptation/uboot/cmd_bin all
ddr_training_clean:
	make -C $(srctree)/drivers/ddr/vendor/boot_adaptation/uboot/cmd_bin clean
