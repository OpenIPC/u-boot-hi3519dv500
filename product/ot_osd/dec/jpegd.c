...
void jpegd_write_regs(jpegd_handle handle, jpgd_regs_type *reg_base)
{
...
void jpegd_read_regs(jpegd_handle handle, const jpgd_regs_type *reg_base)
{
    motion_jpeg_obj *jpeg_hdl_ctx = (motion_jpeg_obj *)handle;
...
int jpegd_start_decoding(jpegd_handle handle)
{
    ...
    jpegd_write_msg_info((jpegd_down_msg *)get_jpegd_emar_buf(), &jpegd_hld_ctx->vpu_config);
#else
    jpegd_set_time_out(0, 0xFFFFFFFF); /* 0xFFFFFFFF:time out value */
#endif
    jpegd_write_regs(handle, (jpgd_regs_type *)JPEGD_REGS_ADDR);
    jpegd_start_vpu(0, &jpegd_hld_ctx->vpu_config);
...
void jpegd_finish_decoding(jpegd_handle handle)
{
    ...
    jpegd_read_regs(handle, (jpgd_regs_type *)JPEGD_REGS_ADDR);
    if (jpegd_hld_ctx->vpu_status.int_dec_finish == 0) {
        printf("hardware decoding error! int_statue 0x%x\n", int_statue);
    } else {
