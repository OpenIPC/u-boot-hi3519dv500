...
static void usb3_do_get_descriptor(usb3_pcd_t *pcd)
{
	...
	case UDESC_DEVICE: {
		...
		dev->id_vendor = get_usb_vendor_id();
	...
	case UDESC_STRING: {
		...
		case STRING_MANUFACTURER:
			...
			buf[1] = UDESC_STRING;
			(void)memcpy_s((void *)(buf + 0x2), USB3_STATUS_BUF_SIZE - 0x2,
				(void *)usb3_dev->string_manu, usb3_dev->string_manu_len);
		...
		case STRING_PRODUCT:
			...
			(void)memcpy_s((void *)(buf + 0x2), USB3_STATUS_BUF_SIZE - 0x2,
				(void *)usb3_dev->string_prod, usb3_dev->string_prod_len);
