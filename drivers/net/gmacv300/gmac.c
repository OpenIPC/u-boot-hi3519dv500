...
static int gmac_init(struct eth_device *dev, struct bd_info *bd)
{
	struct gmac_netdev_local *ld = (struct gmac_netdev_local *)dev->priv;
...
static int gmac_register_dev(port_id_t port_id)
{
	...
	if (snprintf_s(bus->name, sizeof(bus->name), sizeof(bus->name) - 1, mdio_bus[port_id]) < 0)
		printf("snprintf_s failed!func:%s, line: %d\n", __func__, __LINE__);
...
int gmac_initialize(struct bd_info const *bis)
{
	int ret;
