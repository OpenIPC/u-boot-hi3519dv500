...
ident_done:
	...
	if (nand_info_ex.type == 0) {
		errno_t err = -1;
		err = memset_s(&nand_info_ex, sizeof(struct mtd_info_ex), 0, sizeof(struct mtd_info_ex));
		if (err != EOK) {
			pr_info("\nnand_info_ex buff memory set zero fail\n");
			return NULL;
		}
		...
		err = memcpy_s(nand_info_ex.ids, MAX_FLASH_ID_LEN, id_data,
				nand_info_ex.id_length);
		if (err != EOK) {
			pr_info("\nnand_info_ex.ids memory copy fail\n");
			return NULL;
		}
		err = strncpy_s(nand_info_ex.name, MAX_FLASH_NAME_LEN, mtd->name,
				sizeof(nand_info_ex.name));
		if (err != EOK) {
			pr_info("\nnand_info_ex.name memory copy fail\n");
			return NULL;
		}
