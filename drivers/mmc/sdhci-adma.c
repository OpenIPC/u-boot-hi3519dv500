...
static void _sdhci_adma_desc(struct sdhci_adma_desc *desc,
...
static struct sdhci_adma_desc *sdhci_adma_desc(struct sdhci_adma_desc *desc,
			    dma_addr_t addr, u16 len, bool end)
{
	u16 split_len;

	if (((uintptr_t)addr & (SDHCI_DMA_BOUNDARY_SIZE - 1)) + len >
		SDHCI_DMA_BOUNDARY_SIZE) {
		split_len = SDHCI_DMA_BOUNDARY_SIZE -
			((uintptr_t)addr & (SDHCI_DMA_BOUNDARY_SIZE - 1));
			      ...
			      struct mmc_data *data, dma_addr_t addr)
{
	...
	while (--i) {
		desc = sdhci_adma_desc(desc, addr, ADMA_MAX_LEN, false);
		addr += ADMA_MAX_LEN;
	...
	flush_cache((dma_addr_t)table,
		    ROUND((desc_count + 1) * sizeof(struct sdhci_adma_desc),
			  ARCH_DMA_MINALIGN));
}
