...
#include <asm/arch-hi3519dv500/platform.h>
#include "bsp_hi3519dv500.c"
#endif
...
{
	return (readl(PERI_CRG_MMC_CLK) & CRG_CLK_SEL_MASK) >> CRG_CLK_SEL_SHIFT;
}

u32 mmc_get_clk_phase(void)
{
	return (readl(PERI_CRG_MMC_DRV_DLL) & CRG_DRV_PHASE_SEL_MASK) >> CRG_DRV_PHASE_SEL_SHIFT;
}
...
void mmc_get_io_info(void)
{
	...
	param0.bits.emmc_cmd_drv = (readl(io_addr + REG_MMC_CMD_IO) & IO_DRV_MASK) >> IO_DRV_BIT;
	param0.bits.emmc_cmd_sl = !!(readl(io_addr + REG_MMC_CMD_IO) & (1 << IO_SL_BIT));
	param0.bits.emmc_clk_drv = (readl(io_addr + REG_MMC_CLK_IO) & IO_DRV_MASK) >> IO_DRV_BIT;
	param0.bits.emmc_clk_sl = !!(readl(io_addr + REG_MMC_CLK_IO) & (1 << IO_SL_BIT));
	param0.bits.emmc_data_drv = (readl(io_addr + REG_MMC_D0_IO) & IO_DRV_MASK) >> IO_DRV_BIT;
	param0.bits.emmc_data_sl = !!(readl(io_addr +REG_MMC_D0_IO) & (1 << IO_SL_BIT));
...
int sdhci_add_port(int index, uintptr_t regbase, u32 type)
{
	...
	if ((type == MMC_TYPE_MMC) && (mmc_is_fast_boot() != QUICK_BOOT_DIS)) {
		/* Full init needed with BOOT_MODE */
...
static void print_mmcinfo(struct mmc *mmc)
{
	...
	printf("    Mode:        %s\n", mmc->strobe_enhanced ? "HS400ES" :
			...
			(mmc->selected_mode == MMC_LEGACY) ? "Legacy" : "HS");
}
...
int bsp_mmc_init(int dev_num)
{
	...
	start = get_timer(0);
	is_quickboot = (mmc_is_fast_boot() != QUICK_BOOT_DIS);
	...
	if (!IS_SD(mmc)) {
		static char *boot_str[] = {"restore", "sleep", "boot", "ds", "tran"};
		char *boot_mode = NULL;

		print_mmcinfo(mmc);

		if (is_quickboot && mmc_get_cur_mode() >= BOOT_MODE &&
			    mmc_get_cur_mode() <= TRAN_MODE) {
			boot_mode = boot_str[mmc_get_cur_mode() - SLEEP_MODE + 1];
		} else if (is_quickboot && mmc_get_cur_mode() == INIT_MODE) {
			boot_mode = boot_str[0];
		} else {
			boot_mode = "";
		}
		printf("    Boot Mode:   %s(%s), %ld ms\n", \
				is_quickboot? "quick" : "normal", \
				boot_mode, get_timer(start));
		if (is_quickboot) {
