...
static inline unsigned int ddr_dmc_cfg_rnkvol(unsigned int dmc_rnks)
{
	return (0x60 + (dmc_rnks << 2)); /* bit operation , 2 : dmc_rnks */
}
...
/* QOS */
static inline unsigned int qos_ddrc_cfg_rnkvol(unsigned int chs, unsigned int rnks)
{
	return (0x4634 + (chs << 4) + (rnks << 2)); /* 4:channels 2:ranks */
}
