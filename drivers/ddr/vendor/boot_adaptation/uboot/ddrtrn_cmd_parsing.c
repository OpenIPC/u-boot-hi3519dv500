...
static struct ddrtrn_training_result g_ddrtr_result; /* DDR training result */
...
static int ddrtrn_cmd_handle(int cmd)
{
	...
	ddrtrn_copy_data((void *)&g_ddrtr_result, result, sizeof(struct ddrtrn_training_result));
 ...
 */
static int do_ddr_training(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	const char *str = NULL;
	int cmd;
