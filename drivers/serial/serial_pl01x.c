...
#ifdef CONFIG_TARGET_HI3519DV500_FAMILY
...
#ifdef CONFIG_TARGET_HI3519DV500_FAMILY
static int is_uart_download_process(void)
{
	unsigned int val;
	val = readl(REG_SELF_BOOT_FALG);
	return (val == SELF_BOOT_UART_NUDE || val == SELF_BOOT_UART_NO_NUDE);
}
#endif

#ifdef CONFIG_TARGET_HI3519DV500_FAMILY
struct serial_baudinfo baudinfo[] = {
	{
		.clksel = UART_CLK_SEL_24M,
		.baudrate = 115200,
		.ibrd = 0xd,
		.fbrd = 0x1,
	},
...
#ifdef CONFIG_TARGET_HI3519DV500_FAMILY
static unsigned int get_serial_baudrate(struct pl01x_regs *regs)
{
	int i;
	unsigned int baudrate;
	unsigned int uart0_clk_sel;
	unsigned int uart0_ibrd;
	unsigned int uart0_fbrd;

	uart0_ibrd = readl(&regs->pl011_ibrd);
	uart0_fbrd = readl(&regs->pl011_fbrd);
	uart0_clk_sel = readl(REG_UART0_CRG) & UART_CLK_SEL_MASK;

	for (i = 0; i < sizeof(baudinfo)/sizeof(baudinfo[0]); i++) {
		if (((uart0_clk_sel & baudinfo[i].clksel) == baudinfo[i].clksel)
				&& (uart0_ibrd == baudinfo[i].ibrd)
				&& (uart0_fbrd == baudinfo[i].fbrd)) {
			baudrate = baudinfo[i].baudrate;
			break;
		}
	}
...
#ifdef CONFIG_TARGET_HI3519DV500_FAMILY
	if (is_uart_download_process()) {
		gd->baudrate = get_serial_baudrate(base_regs);
		return;
	}
#endif
